...
def compute_da_dz(a):
    ...
    '''
    #########################################
    ## INSERT YOUR CODE HEREs
    sp = a.shape[0]
    da_dz = np.zeros((sp,sp))
    for i in range(sp):
        for j in range(sp):
            if i != j:
...
def compute_dz_dW(x,c):
    ...
    for i in range(c):
        if i != 0:
            M = np.concatenate((M,x.transpose()),axis=0)
        else:
            M = x.transpose()
...
def backward(x,y,a):
    ...
    dz_dW = compute_dz_dW(x,sp)
    dz_db = compute_dz_db(sp)
